...
class MotorController(basic.LineOnlyReceiver):
  ...
  def sendLine(self, line):
    logging.debug('Controller %s sending: %s', self.serial_port, line)
    if self.simulate:
  ...
  def lineReceived(self, line):
    if not line:
      return
    logging.debug('Controller %s received: %s', self.serial_port, line)
  ...
  def go_to_position(self, channel, value):
